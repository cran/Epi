      ...
      print.AOV = TRUE )
{
    ...
    if (length(grep("-", parm)) == 0) {
        ...
        drift <- ci.lin(adc, subset = "I\\(", Exp = TRUE)[, 5:7,
            drop = F]
        rownames(drift) <- "A-d"
        xP <- cbind(1, P - p0, MPr)
        ...
        if (parm == "AD-C-P") {
            ...
            rp <- update(m.0, . ~ . - 1 + xP, offset = predict(adc,
                type = "link"))
            A.eff <- ci.lin(adc, subset = "MA", ctr.mat = MA[A.pos,
                ], Exp = TRUE, alpha = alpha)[, 5:7]
            C.eff <- ci.lin(rc, subset = "xC", ctr.mat = xC[C.pos,
                ], Exp = TRUE, alpha = alpha)[, 5:7]
            P.eff <- ci.lin(rp, subset = "xP", ctr.mat = xP[P.pos,
                ], Exp = TRUE, alpha = alpha)[, 5:7]
        }
        else if (parm == "AD-P-C") {
            ...
            rc <- update(m.0, . ~ . - 1 + xC, offset = predict(rp,
                type = "link"))
            A.eff <- ci.lin(adp, subset = "MA", ctr.mat = MA[A.pos,
                ], Exp = TRUE, alpha = alpha)[, 5:7]
            P.eff <- ci.lin(rp, subset = "xP", ctr.mat = xP[P.pos,
                ], Exp = TRUE, alpha = alpha)[, 5:7]
            C.eff <- ci.lin(rc, subset = "xC", ctr.mat = xC[C.pos,
            ...
            rp <- update(m.0, . ~ . - 1 + xP, offset = predict(ac,
                type = "link"))
            A.eff <- ci.lin(ac, subset = "MA", ctr.mat = MA[A.pos,
                ], Exp = TRUE, alpha = alpha)[, 5:7]
            C.eff <- ci.lin(ac, subset = "lC", ctr.mat = lC[C.pos,
                ], Exp = TRUE, alpha = alpha)[, 5:7]
            P.eff <- ci.lin(rp, subset = "xP", ctr.mat = xP[P.pos,
                ], Exp = TRUE, alpha = alpha)[, 5:7]
        }
            ...
            rc <- update(m.0, . ~ . - 1 + xC, offset = predict(ap,
                type = "link"))
            A.eff <- ci.lin(ap, subset = "MA", ctr.mat = MA[A.pos,
                ], Exp = TRUE, alpha = alpha)[, 5:7]
            P.eff <- ci.lin(ap, subset = "lP", ctr.mat = lP[P.pos,
                ], Exp = TRUE, alpha = alpha)[, 5:7]
            C.eff <- ci.lin(rc, subset = "xC", ctr.mat = xC[C.pos,
                ], Exp = TRUE, alpha = alpha)[, 5:7]
        }
