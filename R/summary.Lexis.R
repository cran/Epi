     ...
     timeScales = FALSE, ... )
{
    ...
    if (!is.null(by)) {
        if (is.character(by)) {
            ...
            else res <- lapply(split(object, object[, by]),
                               summary.Lexis,
                               by = NULL, # mat = FALSE,
            ...
            else res <- lapply(split(object, by),
                               summary.Lexis,
                               by = NULL, # mat = FALSE,
                               simplify = simplify, scale = scale,
                               Rates = Rates, timeScales = timeScales, ...)
...
if (!Rates     ) res <- res[-2]
# return as a matrix if feasible
# if (length(res) == 1 & mat) res <- res[[1]]
