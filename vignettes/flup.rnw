...
This vignette uses a data example from the \texttt{Epi} package to
...
\begin{minipage}[t]{0.6\linewidth}
  \raggedright
The \code{Lexis} functions(s) are named after the German demographer
and economist Wilhelm Lexis (full name Wilhelm Hector Richard Albrecht
Lexis) (17 July 1837 -- 24 August 1914), who in his book ``Einleitung
in die Theorie der Bev\"{o}lkerungsstatistik'' [Introduction to the
theory of population statistics] (1875) devised a diagram showing
follow-up of persons on multiple time scales, notably calendar time
and age. The diagram that nowadays is called a Lexis diagram, is
usually drawn in a slightly different manner than that Lexis used in
his book.

The display of follow-up on two timescales naturally leads to
...
terms. Hence the naming of the machinery that represents follow-up on
multiple time scales after Wilhelm Lexis.
\end{minipage}
\hfill
...
likelihood could be approximated by a Poisson likelihood. This is
basically what underlies the time-splitting and subsequent modeling of
transition rates in this
book.
\noindent\\[2em]
...
\item[\texttt{AaJ.Lexis}] compute the Aalen-Johansen estimator for a
  ...
  \texttt{survival}
\item[\texttt{ci.Crisk}] compute cumulative risks from model objects
  for rates
...
\item[\texttt{coxphLexis}] fit a Cox model to follow-up in a
  \texttt{Lexis} object
\item In versions of Epi up to 2.56 these functions were called
  \texttt{glm.Lexis},
  \texttt{gam.Lexis} and
  \texttt{coxph.Lexis} --- but they are not S3 methods so naming was
