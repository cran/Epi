...
function( x, ... )
{
# This function converts a MethComp object which inherits from
# mcmc.list to a MethComp object that inherits from mcmc by
# lumping together the chains
if( !inherits( x, "MethComp" ) ) stop( "The argument (here:",
                                       deparse( substitute( x ) ),
                                       ") must be a MethComp object." )
res <- as.mcmc( as.matrix( x ) )
attributes( res ) <- attributes( x )
class( res ) <- c("MethComp","mcmc")
res
}
